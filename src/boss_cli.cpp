...
void ShowVersion() {
	...
	std::cout << bloc::translate("Version ") << BOSS_VERSION_MAJOR << "." << BOSS_VERSION_MINOR << "." << BOSS_VERSION_PATCH << std::endl;
}
...
void ShowUsage(po::options_description opts) {
	...
	std::cout << std::endl << bloc::translate("Description:") << std::endl
	          ...
	          << "  " << progName << " -u" << std::endl
	          << bloc::translate("    updates the masterlist, sorts your mods, and shows the log") << std::endl << std::endl
	          << "  " << progName << " -sr" << std::endl
	          << bloc::translate("    reverts your load order 1 level and skips showing the log") << std::endl << std::endl
	          << "  " << progName << " -r 2" << std::endl
	          << bloc::translate("    reverts your load order 2 levels and shows the log") << std::endl << std::endl;
}
...
int progress(const git_transfer_progress *stats, void *payload) {
	std::printf((bloc::translate("Downloading masterlist: %i of %i objects (%i KB)").str() + "\r").c_str(),
...
int bossMain(int argc, char *argv[]) {
	Settings ini;
	Game game;
	...
	try {
		...
		if (detectedGame == AUTODETECT) {
			...
			if (detected.empty()) {
				...
				std::cout << std::endl << bloc::translate("Please pick which game to run BOSS for:") << std::endl;
				for (std::size_t i = 0; i < detected.size(); i++)
					std::cout << i << " : " << Game(detected[i], "", true).Name() << std::endl;

				std::cin >> ans;
				if (ans < 0 || ans >= detected.size()) {
					std::cout << bloc::translate("Invalid selection.") << std::endl;
					throw boss_error(BOSS_ERROR_NO_GAME_DETECTED);
				}
		...
		game.bosslog.criticalError << LIST_ITEM_CLASS_ERROR << bloc::translate("Critical Error: ") << e.getString() << LINE_BREAK
	...
	if (gl_revert < 1 && (gl_update || gl_update_only)) {
		std::cout << std::endl << bloc::translate("Updating to the latest masterlist from the online repository...") << std::endl;
		LOG_DEBUG("Updating masterlist...");
		try {
			std::string revision = UpdateMasterlist(game, progress, NULL);
			std::string message = (boost::format(bloc::translate("Masterlist updated; at revision: %1%.")) % revision).str();
			game.bosslog.updaterOutput << LIST_ITEM_CLASS_SUCCESS << message;
			std::cout << std::endl << message << std::endl;
		} catch (boss_error &e) {
			game.bosslog.updaterOutput << LIST_ITEM_CLASS_ERROR << bloc::translate("Error: masterlist update failed.") << LINE_BREAK
			                           << (boost::format(bloc::translate("Details: %1%")) % e.getString()).str() << LINE_BREAK;
			LOG_ERROR("Error: masterlist update failed. Details: %s",
			          e.getString().c_str());
		}
	} else {
		std::string revision = GetMasterlistVersion(game);
		std::string message = (boost::format(bloc::translate("Masterlist updating disabled; at revision: %1%.")) % revision).str();
		game.bosslog.updaterOutput << LIST_ITEM_CLASS_SUCCESS << message;
	}
	...
	std::cout << std::endl << bloc::translate("BOSS working...") << std::endl;
...
int main(int argc, char *argv[]) {
	return boss::bossMain(argc, argv);
}
